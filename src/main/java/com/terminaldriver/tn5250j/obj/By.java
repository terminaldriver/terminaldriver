...
public abstract class By {
  ...
  /**
   * @param labelText The exact text to match against
  ...
  public static By and(final By ... by ) {
	    if (by.length== 0)
	      throw new IllegalArgumentException(
	          "Cannot find elements when compound By is empty.");
  ...
  public static By row(final int row) {
	    if (row < 1)
	      throw new IllegalArgumentException(
	          "Cannot find elements when row is invalid.");
  ...
  public static By column(final int column) {
	    if (column < 1)
	      throw new IllegalArgumentException(
	          "Cannot find elements when column is invalid.");
  ...
  public ScreenElement findElement(TerminalDriver driver) {
    List<ScreenElement> allElements = findElements(driver);
  ...
  public static class ByLabelText extends By implements Serializable {
	    ...
	    public ByLabelText(String labelText, Position position) {
	        this.labelText = labelText;
  ...
  public static class ByAttribute extends By implements Serializable {
	    ...
	    public ByAttribute(ScreenAttribute attribute) {
  ...
  public static class ByPosition extends By implements Serializable {
	    ...
	    public ByPosition(Integer row, Integer column) {
		      this.row = row;
		      this.column = column;
     }
		...
		public boolean matches(ScreenElement element) {
			return 
					(column == null || element.startCol()==column) 
					&& (row == null || element.startRow()==row);
		}
 }
  ...
  public static class ByAnd extends By implements Serializable {
    ...
    @Override
    public List<ScreenElement> findElements(TerminalDriver driver) {
      List<ScreenElement> x = bys.get(0).findElements(driver);
    ...
    @Override
    public ScreenElement findElement(TerminalDriver driver) {
    	List<ScreenElement> x =findElements(driver);
    	if(x.isEmpty()){
    		return null;
    	}else{
    		return x.get(0);
